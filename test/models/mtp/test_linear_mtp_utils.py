import unittest
import os
from typing import List

import torch

from ai2pot.models.mtp.linear_mtp_utils import LinearMtpCalculator



TEST_FILES_DIR = os.getenv("AI2POT_PATH")
CHECK_POINT_PATH: str = os.path.join(TEST_FILES_DIR,
                                     "lightning_logs",
                                     "linear_mtp",
                                     "version_34",
                                     "checkpoints",
                                     "epoch=146-step=3675.ckpt")


class LinearMtpCalculatorTest(unittest.TestCase):
    def setUp(self):
        print("LinearMtpCalculatorTest (TestSuite) is setting up...")
        self.checkpoint_path: str = CHECK_POINT_PATH
        self.testset_path: str = os.path.join(TEST_FILES_DIR,
                                              "test",
                                              "test_data",
                                              "XYZ",
                                              "11_NEP_potential_PbTe",
                                              "test_m.xyz")
        self.has_virial: bool = False
        self.map_location: str = "cpu"        
        self.torch_float_dtype: torch._C.dtype = torch.float32
        self.linear_mtp_calculator: LinearMtpCalculator = LinearMtpCalculator(checkpoint_path=self.checkpoint_path,
                                                                              testset_path=self.testset_path,
                                                                              map_location=self.map_location,
                                                                              torch_float_dtype=self.torch_float_dtype)

    def tearDown(self):
        print("LinearMtpCalculatorTest (TestSuite) is tearing down...")

    
    def test_calculate_ef_rmse(self):
        print(self.linear_mtp_calculator.calculate_ef_rmse())


    def test_calculate_efv_rmse(self):
        self.linear_mtp_calculator.calculate_efv_rmse()



if __name__ == '__main__':
    unittest.main()
