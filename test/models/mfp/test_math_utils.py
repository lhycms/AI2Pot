...
class MathUtilsTest(unittest.TestCase):
    ...
    def test_build_cart_basis(self):
        ...
        print(" 1.1. Mean norm of l0 basis = ", torch.sum(B0*B0, dim=1).mean())
        print(" 1.2. Mean norm of l1 basis = ", torch.sum(B1*B1, dim=1).mean())
        print(" 1.3. Mean norm of l2 basis = ", torch.sum(B2*B2, dim=1).mean())
