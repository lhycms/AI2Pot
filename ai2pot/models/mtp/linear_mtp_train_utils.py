...
class LinearMtpDescriptorNormCallback(L.Callback):
    def on_fit_start(self,
                     trainer: L.Trainer,
                     pl_module: L.LightningModule):
        print("🚀🚀🚀 Performing linear mtp descriptor normalization before training -- Starting...")

        datamodule: ExtxyzDataModule = trainer.datamodule
        linear_mtp: LinearMtp = pl_module.model
        linear_mtp_stat: LinearMtp = copy.deepcopy(linear_mtp)
        ...
        train_loader: DataLoader = DataLoader(dataset=trainset,
                                              batch_size=500,
                                              shuffle=False)
        num_descriptors: int = linear_mtp.get_num_descriptors()
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(train_loader):
                ...
                bdescriptor_tensor: torch.Tensor = linear_mtp_stat.predict_descriptors(binum, 
                                                                                       bilist, 
                                                                                       bnumneigh, 
                                                                                       bfirstneigh, 
                                                                                       brcs, 
...
class FullParametersOptimizer(object):
    def __init__(self,
