...
for (int aa=0; aa<3; aa++)
    for (int bb=0; bb<3; bb++)
        printf("%.8lf, ", bvirial_[aa*3+bb]);
printf("]\n");
}


TEST_F(NepTest, find_loss_backward_launcher)
{
    real e_weight = 0.1;
    real f_weight = 0.2;
    real v_weight = 0.0;

    ai2pot::nep::find_efv_launcher<real>(
        betot,
        bforce,
        bvirial,
        chebyshev_size,
    ...
    ai2pot::nep::find_loss_backward_launcher<real>(
        bloss_der2coeffs,
        bloss_der2w0,
        bloss_der2b0,
        bloss_der2w1,
