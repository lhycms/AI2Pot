...
class NNMtpCPULauncher : public :: testing::Test
{
protected:
    int ntypes;
    ...
    void SetUp() override {
        ...
        // Loss derivative
        bloss_der2coeffs = (real*)malloc(sizeof(real) * ntypes * ntypes * mtp_param.nmus() * chebyshev_size);
        memset(bloss_der2coeffs, 0, sizeof(real) * ntypes * ntypes * mtp_param.nmus() * chebyshev_size);
        bloss_der2w0 = (real*)malloc(sizeof(real) * ntypes * num_neurons * mtp_param.alpha_scalar_moments());
        memset(bloss_der2w0, 0, sizeof(real) * ntypes * num_neurons * mtp_param.alpha_scalar_moments());
        bloss_der2b0 = (real*)malloc(sizeof(real) * ntypes * num_neurons);
        memset(bloss_der2b0, 0, sizeof(real) * ntypes * num_neurons);
        bloss_der2w1 = (real*)malloc(sizeof(real) * ntypes * num_neurons);
        memset(bloss_der2w1, 0, sizeof(real) * ntypes * num_neurons);
        bloss_der2type_bias = (real*)malloc(sizeof(real) * ntypes);
...
TEST_F(NNMtpCPULauncher, find_ef_cpu_launcher) {
...
TEST_F(NNMtpCPULauncher, find_efv_cpu_launcher) {
    int center_idx_modify = 1;
    int direction1_idx_modify = 2;
...
TEST_F(NNMtpCPULauncher, find_descriptors_cpu_launcher) {
    int center_idx_modify = 0;

    ai2pot::nnmtp::find_descriptors_cpu_launcher<real>(
        bdescriptors,
        chebyshev_size,
        coeffs,
        mtp_param.alpha_moments_count(),
...
printf("1.1. descriptors of atom#%d = \n\t[", center_idx_modify);
for (int ii=0; ii<mtp_param.alpha_scalar_moments(); ii++)
