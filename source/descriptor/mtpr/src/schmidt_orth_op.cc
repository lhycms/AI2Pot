...
torch::autograd::variable_list CoeffsSchmidtOrthFunction::forward(
    ...
    const at::Tensor& coeffs_tensor)
{
    ...
    if (coeffs_tensor.scalar_type() == torch::kFloat32) {
        ...
        if (coeffs_tensor.device() == c10::kCPU) {
...
torch::autograd::variable_list CoeffsSchmidtOrthFunction::backward(
    torch::autograd::AutogradContext *ctx,
...
torch::autograd::variable_list CoeffsSchmidtOrthOp(
    int ntypes,
    int nmus,
    int chebyshev_size,
    const at::Tensor& coeffs_tensor)
{
    return CoeffsSchmidtOrthFunction::apply(
        ntypes,
        nmus,
        chebyshev_size,
        coeffs_tensor);
