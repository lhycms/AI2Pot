    ...
    CoordType *dei_drij)
{
    ...
    memset(virial_sr_der1, 0, sizeof(CoordType) * 3 * 3 * inum * umax_num_neighs * 3);
    memset(virial_sr_der2, 0, sizeof(CoordType) * 3 * 3 * inum * umax_num_neighs * 3);
        ...
        for (int jj=0; jj<numneigh[ii]; jj++) {
            for (int aa=0; aa<3; aa++) {
                for (int bb=0; bb<3; bb++) {
                    ...
                    virial_sr_val[aa*3 + bb] -= dei_drij[tmp_idx1] * rcs[tmp_idx2];
                    virial_sr_der1[(aa*3+bb)*inum*umax_num_neighs*3 + tmp_idx1] -= rcs[tmp_idx2];
                    virial_sr_der2[(aa*3+bb)*inum*umax_num_neighs*3 + tmp_idx2] -= dei_drij[tmp_idx1];
                }
            }
        }
    }
